...
/* Question-to-question galaxy zoom transition.
   The ENTIRE question travels through space: the old question flies
   away into the distance, the stars warp around it, and the new
   question starts far away before flying toward the student.
   Keep this value in sync with the .question-content transition in style.css. */
...
const QUESTION_TRANSITION_MS   = 650;  // cinematic zoom-out / zoom-in time for the floating question
...
/* ------------------------------------------------------------
   ...
   results screen), the current question zooms out and fades away
   like it's drifting off into space, a quick burst of hyperspace
   stars flies past, and the next question zooms in from the
   distance to settle into place — the same galaxy effect as the
   opening entrance, just a shorter hop instead of a full journey.
   ------------------------------------------------------------ */
...
// keep it roughly in line with QUESTION_TRANSITION_MS * 2.
...
function playGalaxyZoomTransition(exitEl, enterEl, onSwap) {
  ...
  playQuestionWarpBurst(QUESTION_TRANSITION_MS * 2 - 60);

  // 1) Current question becomes a distant object and flies away.
  //    The whole questionContent wrapper transforms, not just its children.
  ...
  setTimeout(() => {
    ...
    void enterEl.offsetWidth; // force reflow: place the new question deep in space
    requestAnimationFrame(() => {
      // 3) The new question now flies toward the student and settles.
